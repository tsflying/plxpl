...
<body>
	...
	<input type="hidden" id="prodcutId" value="${product.id }"/>
		<ul>
			<li><span>产品名称:</span><input id="productName"></li>
			<li><span>产品单价:</span><input id="productPrice"></li>
			<li><span>产品图片：</span><img id="imgSrc"/></li>
	...
	<div style="width:50%;float:left; ">
		<ul>
			<li><span>产品描述：</span><textarea rows="3" cols="20" id="productBody"></textarea></li>
			<li><span>产品详情:</span><textarea rows="3" cols="20" id="productDetail"></textarea></li>
			<li><span>产品附加信息：</span><textarea rows="3" cols="20" id="productAttach"></textarea></li>
		</ul>
	</div>
	...
	<script type="text/javascript">
		function save(){
			...
			a.splice(0,2);
			imgSrc = a.join("/");
			var productBody = $("#productBody").val();
			var productDetail = $("#productDetail").val();
			var productAttach = $("#productAttach").val();
			
			$.ajax( {  
			    url:'<%=path%>/backend/addProductInfo.do',// 跳转到 action  
			    data:{  
			    		 id : prodcutId,
			             name : productName,  
			             price : productPrice,  
